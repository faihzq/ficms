...
/**
 * Report17SurveyController implements the CRUD actions for Report17Survey model.
 */
class Report17SurveyController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all Report17Survey models.
     *
     * @return string
     */
    ...
    /**
     * Displays a single Report17Survey model.
     * @param int $id ID
     * @return string
    ...
    /**
     * Creates a new Report17Survey model.
    ...
    public function actionCreate()
    {
        ...
        $listReport17Defect = ArrayHelper::map(Report17Defect::find()->where(['IN', 'status_id', [2]])->all(), 'id', 'report_no');
    ...
    /**
     * Updates an existing Report17Survey model.
    ...
    public function actionSign($id, $section)
    {
        ...
        if ($this->request->isPost && $model->load($this->request->post())) {
            ...
            if ($section == 1){
                ...
                $model->commander_sign= Yii::$app->request->post('Report17Survey')['commander_sign'];
                // set updated time
                $model->updated_time = $time;
                $model->updated_user_id = Yii::$app->user->identity->id;
            ...
            if ($model->save(false)){
                ...
                if ($model->status_id == 4 && $model->commander_sign_status_id == 1){
                    $modelRepair = new Report17Repair();
    ...
    public function actionBoat($id)
    {
        if (Yii::$app->request->isAjax) {
            ...
            $data = Report17Defect::findOne($selectedValue);
            if ($data) {
                $response = [
    ...
    /**
     * Deletes an existing Report17Survey model.
    ...
    /**
     * Finds the Report17Survey model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return Report17Survey the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Report17Survey::findOne(['id' => $id])) !== null) {
            return $model;
        }
