...
class Report17RepairController extends Controller
{
    ...
    {
        $model = Report17Repair::find()->orderBy(['created_time' => SORT_DESC])->all();
    ...
    public function actionView($id)
    {
        ...
        $modelReportStatusLog = ReportStatusLog::find()->where(['report_id'=>$id])->andWhere(['report_type_id'=>3])->andWhere(['report_no'=>2])->orderBy(['updated_time'=>SORT_DESC])->all();
    ...
    public function actionTask()
    {
        if (Yii::$app->user->identity->user_role_id == 1){
            $model1 = Report17Repair::find()->where(['=', 'status_id', 4])->andWhere(['=', 'engineer_sign_status_id', 0])->orderBy(['created_time' => SORT_DESC])->all();
            $model2 = Report17Repair::find()->where(['=', 'status_id', 4])->andWhere(['=', 'engineer_sign_status_id', 1])->andWhere(['=', 'commander_sign_status_id', 0])->orderBy(['created_time' => SORT_DESC])->all();
            $model = array_merge($model1, $model2);
        } else if (Yii::$app->user->identity->user_role_id == 3 || Yii::$app->user->identity->user_role_id == 2){
            $model = Report17Repair::find()->where(['=', 'status_id', 4])->orderBy(['created_time' => SORT_DESC])->all();
        } else {
            $model = Report17Repair::find()->where(['=', 'status_id', 4])->andWhere(['=', 'engineer_sign_status_id', 1])->andWhere(['=', 'commander_sign_status_id', 0])->orderBy(['created_time' => SORT_DESC])->all();
        }
        $listStatus = ArrayHelper::map(ReportStatus::find()->all(), 'name', 'name');
