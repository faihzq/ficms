...
/**
 * This is the model class for table "report17_survey".
 *
 * @property int $id
 ...
 * @property int $damage_type_survey_id
 ...
 */
class Report17Survey extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    public function attributeLabels()
    {
        return [
            'id' => 'ID',
            'report_damage_id' => 'Rujukan No. Laporan Latent Defect',
            'report_date' => 'Tarikh',
            ...
            'damage_description' => 'Keterangan LD',
            'probable_cause' => 'Sebab Kemungkinan',
            'boat_status_id' => 'Status Bot',
            'warranty_protection' => 'Tanggungan LD',
    ...
    public static function getTaskCounter()
    {
        ...
        if (Yii::$app->user->identity->user_role_id == 1){
            $model1 = Report17Survey::find()->where(['=', 'status_id', 2])->andWhere(['=', 'engineer_sign_status_id', 0])->count();
            $model2 = Report17Survey::find()->where(['=', 'status_id', 2])->andWhere(['=', 'engineer_sign_status_id', 1])->andWhere(['=', 'commander_sign_status_id', 0])->count();
            $counter = $model1 + $model2;
        } else if (Yii::$app->user->identity->user_role_id == 3 || Yii::$app->user->identity->user_role_id == 2){
            $counter = Report17Survey::find()->where(['=', 'status_id', 2])->count();
        } else {
            $counter = Report17Survey::find()->where(['=', 'status_id', 2])->andWhere(['=', 'engineer_sign_status_id', 1])->andWhere(['=', 'commander_sign_status_id', 0])->count();
        }
    ...
    {
        return $this->hasOne(Report17Defect::className(),['id'=>'report_damage_id']);
    }
    ...
    public function base64_to_png_eng() {
        ...
        $path = 'uploads/report17Survey/sign/';
        if (!file_exists($path)) {
